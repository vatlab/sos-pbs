...
RUN  pip install fasteners pygments ipython ptpython networkx pydotplus sos sos-notebook sos-bash
...
#
SHA=$(curl -s 'https://api.github.com/repos/vatlab/sos-pbs/commits' | grep sha | head -1 | cut -d\" -f4)
echo "COMMIT SHA $SHA"
